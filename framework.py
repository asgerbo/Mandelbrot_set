...
class Mandelbrot_Viewer(Frame):
    """
    This is the main class for the Mandelbrot Viewer. I use the __init__ statement to initialize the class. The super function states that this is a child class which comes from the frame class. This allows me to use tkinter methods and options that are common to all tkinter widgets. This one is a bit different, since we have set values for the x_center, y_center, scale, max_iter, img_width, img_height and use_multiprocessing. Using x_center = -0.75, y_center = 0 and scale = 2, seems to be good starting points after having played around with the values. A scale too high, and the set will be too small, and a scale too low, and the set will appear too big. 
    """
    def __init__(self, parent, canvas_size, x_center = -0.5, y_center = 0, scale = 2, max_iter = None, img_width = None, img_height = None,  use_multiprocessing=True):
...
def run_program():
    """
    This is the main function. I use the freeze_support function from the multiprocessing package to prevent the program from crashing when running the program on Windows. I then initialize the root window and set the size of the window. I then initialize the Mandelbrot_Viewer class and set the size of the window. I then call the mainloop function to run the program.
    """
    root = Tk()
